...
| 22 | `factory(()=> inject(ServiceE))`\* | `ServiceD`
| ... | ... | ...
...
We should treat these special objects like any other token. `DIRECTIVE_INJECTOR` already reads a special `NG_ELEMENT_ID`
property set on directives to locate their bit in the bloom filter. We can set this same property on special objects,
but point to a factory function rather than an element ID number. When we check that property in `DIRECTIVE_INJECTOR`
...
Consequence of the above is that `DIRECTIVE_INJECTOR` returns an instance of `ElementRef` without `Injector` having to know about `ElementRef` at compile time.
