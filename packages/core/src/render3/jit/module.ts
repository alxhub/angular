...
import {Expression, R3InjectorMetadata, R3NgModuleMetadata, WrappedNodeExpr, compileInjector, compileNgModule as compileR3NgModule, jitExpression, R3Reference} from '@angular/compiler';
    ...
    get: () => {
      if (ngModuleDef === null) {
        const meta: R3NgModuleMetadata = {
          ...
          imports:
              flatten(ngModule.imports || EMPTY_ARRAY).map(expandModuleWithProviders).map(wrapReference),
          exports:
              flatten(ngModule.exports || EMPTY_ARRAY).map(expandModuleWithProviders).map(wrapReference),
          emitInline: true,
        };
