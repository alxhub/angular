...
export interface NestedSignalNode extends ComputedNode<unknown> {
  rawDirty: boolean;
...
export const NESTED_SIGNAL_NODE: Omit<NestedSignalNode, 'computation' | 'parentNode'> =
  /* @__PURE__ */ (() => {
    return {
      ...COMPUTED_NODE,
      kind: 'nestedSignal',
      ...
      rawDirty: false,
      get dirty(): boolean {
        if (isSafeToShortCircuitNotifications(this as NestedSignalNode)) {
 ...
 */
function isSafeToShortCircuitNotifications(node: NestedSignalNode): boolean {
  // If the current write path doesn't involve any nested signal writes, then we're potentially affected.
  if (nestedSignalWriter === null) {
    return false;
  }

  // `parentNode` is the most recent nested signal being written. If this node isn't also a child
  // of that parent, then we can't guarantee we're not impacted by the write.
  if (nestedSignalWriter.parentNode !== node.parentNode) {
    return false;
  }
  ...
  if (nestedSignalWriter.lastProperty === node.lastProperty) {
    return false;
  }

  // We've proven that:
  //  * another nested signal is changing a specific property of our parent signal
...
export function setNestedSignalWriter(node: NestedSignalNode | null): NestedSignalNode | null {
  const prev = nestedSignalWriter;
  nestedSignalWriter = node;
  return prev;
}

let nestedSignalWriter: NestedSignalNode | null = null;
