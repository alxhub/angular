...
import {NESTED_SIGNAL_NODE, type NestedSignalNode, setNestedSignalWriter} from './internal';

export interface NestedSignalOptions {
  debugName?: string;
}

export function nestedSignal<T extends {}, K extends keyof T>(
  parent: WritableSignal<T>,
  key: K | Signal<K>,
  options?: NestedSignalOptions,
): WritableSignal<T[K]> {
  type V = T[K];

  let keySignal: Signal<K> | undefined;

  // Create our `NESTED_SIGNAL_NODE` and initialize it.
  const node: NestedSignalNode = Object.create(NESTED_SIGNAL_NODE);
  if (isSignal(key)) {
    keySignal = key;
  ...
  // The nested signal specific write path.
  getter.set = (value: V) => {
    ...
    // exist, we don't use the `nestedSignal` write path. This ensures any `structuralSignal`s will
    // get notified of this change.
    let prevWriter: NestedSignalNode | null;
    if ((node.lastProperty as K) in current) {
      prevWriter = setNestedSignalWriter(node);
    } else {
      prevWriter = setNestedSignalWriter(null);
    }
    ...
    try {
      ...
      // `nestedSignal`s is updated. We probably want to delay that until we come back up the stack
      // and finish the write path of every `nestedSignal` along the way (marking clean, etc).

      // Unlike normal computed nodes, nested signals handle updating their own value. Therefore, once
      // we set the value we can consider ourselves clean. We could set our value earlier.
      node.value = value;
      producerMarkClean(node);
    } finally {
      setNestedSignalWriter(prevWriter);
    }
  };
  ...
  if (typeof ngDevMode !== 'undefined' && ngDevMode) {
    ...
    getter.toString = () => `[nestedSignal${debugName}: ${node.value}]`;
  }
