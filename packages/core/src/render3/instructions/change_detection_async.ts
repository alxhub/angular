...
export function* detectChangesInternalAsync(
  lView: LView,
  ...
  try {
    yield* detectChangesInViewWhileDirty(lView, mode);
  } catch (error) {
...
function* detectChangesInViewWhileDirty(lView: LView, mode: ChangeDetectionMode) {
  ...
  try {
    ...
    yield* detectChangesInView(lView, mode);
    ...
    while (requiresRefreshOrTraversal(lView)) {
      ...
      yield* detectChangesInView(lView, ChangeDetectionMode.Targeted);
    }
  } finally {
...
function* refreshView<T>(
  ...
  const state = {
    lFrame: saveLFrame(),
    consumer: getActiveConsumer(),
    isRefreshingViews: isRefreshingViews(),
  };

  yield;
  setActiveConsumer(state.consumer);
  restoreLFrame(state.lFrame);
  setIsRefreshingViews(state.isRefreshingViews);
  ...
  try {
    ...
    yield* detectChangesInEmbeddedViews(lView, ChangeDetectionMode.Global);
    ...
    if (components !== null) {
      yield* detectChangesInChildComponents(lView, components, ChangeDetectionMode.Global);
    }
...
function* detectChangesInEmbeddedViews(lView: LView, mode: ChangeDetectionMode) {
  for (
    ...
    for (let i = CONTAINER_HEADER_OFFSET; i < lContainer.length; i++) {
      ...
      yield* detectChangesInViewIfAttached(embeddedLView, mode);
    }
  }
}
...
function* detectChangesInComponent(
  ...
  yield* detectChangesInViewIfAttached(componentView, mode);
}
 ...
 */
function* detectChangesInView(lView: LView, mode: ChangeDetectionMode): Generator {
  const isInCheckNoChangesPass = ngDevMode && isInCheckNoChangesMode();
  ...
  if (shouldRefreshView) {
    yield* refreshView(tView, lView, tView.template, lView[CONTEXT]);
  } else if (flags & LViewFlags.HasChildViewsToRefresh) {
    yield* detectChangesInEmbeddedViews(lView, ChangeDetectionMode.Targeted);
    const components = tView.components;
    if (components !== null) {
      yield* detectChangesInChildComponents(lView, components, ChangeDetectionMode.Targeted);
    }
  }
}
...
function* detectChangesInChildComponents(
  ...
  for (let i = 0; i < components.length; i++) {
    yield* detectChangesInComponent(hostLView, components[i], mode);
  }
}
