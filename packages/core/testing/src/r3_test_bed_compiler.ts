...
export class R3TestBedCompiler {
  ...
  // keep track of all components and directives,
  // so we can patch Providers onto defs later.
  ...
  // TODO: what about multiple defs on a type
  private initialNgDefs = new Map<Type<any>, [string, PropertyDescriptor|undefined]>();

  // Map that keeps cleanup functions for initial versions of component/directive/pipe/module defs
  // in case TestBed makes changes to the originals.
  ...
  setCompilerProviders(providers: StaticProvider[]|null): void {
    ...
    // TODO: not sure about this? we shouldn't mix compiler providers with normal providers
    // this.providerOverrides.push(...providers);
    this._injector = null;
  }
  ...
  overrideModule(ngModule: Type<any>, override: MetadataOverride<NgModule>): void {
    ...
    // new
    // declarations or imported modules. Ingest any possible new types and add them to the current
    // queue.
  ...
  overrideTemplateUsingTestingModule(type: Type<any>, template: string): void {
    ...
    // scope, so overrideTemplateUsingTestingModule can be implemented purely via
    // overrideComponent.
  ...
  private applyProviderOverridesToModule(moduleType: Type<any>): void {
    ...
    if (this.hasProviderOverrides(def.providers)) {
      ...
      def.providers = this.getProvidersOverrides(def.providers);
    }
  }
  ...
  private queueType(type: Type<any>, moduleType: Type<any>|TESTING_MODULE): void {
    ...
    if (component) {
      // TODO: add hasOwnProperty checks for Directive and Pipes!
      // Broken case: class with NO annotations extends class WITH annotations.
  ...
  restoreOriginalState(): void {
    ...
    this.initialNgDefs.forEach((value: [string, PropertyDescriptor], type: Type<any>) => {
      ...
      if (!descriptor) {
        ...
        // invoked should be quite rare to cause any noticable impact, since it's applied only
        // to
        // some test cases (for example when class with no annotations extends some @Component)
        // when
        // we need to clear 'ngComponentDef' field on a given class to restore its original
        // state
  ...
  get injector(): Injector {
    ...
    // TODO(ocombe): make this work with an Injector directly instead of creating a module for
    // it
    @NgModule({providers})
  ...
  // TODO: give a better name to `getProviderOverrides` and `getProvidersOverrides` fns
  private getProviderOverrides(provider: any) {
  ...
  private getProvidersOverrides(providers: any): any[] {
    ...
    return flatten(flatten(providers, (provider: any) => this.getProviderOverrides(provider)));
  }

  private hasProviderOverrides(providers: any): boolean {
    return this.getProvidersOverrides(providers).length > 0;
  }
  ...
  private patchDefWithProviderOverrides(declaration: Type<any>, field: string) {
    ...
    if (def && def.providersResolver) {
      ...
      const processProvidersFn = (providers: any[]) => {
        const overrides = this.getProvidersOverrides(providers);
