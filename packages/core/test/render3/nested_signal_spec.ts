...
import {nestedSignal} from '../../src/render3/reactivity/nested_signal/nested_signal';
import {structuralSignal} from '../../src/render3/reactivity/nested_signal/structural_signal';

describe('nestedSignal', () => {
  it('propagates an update from parent -> child correctly', () => {
    const parent = signal({name: 'John'});
    const child = nestedSignal(parent, 'name');
  ...
  it('does not block updates to normal computed children', () => {
    const parent = signal({name: 'John'});
    const nestedName = nestedSignal(parent, 'name');
  ...
  it('blocks updates to siblings of a parent', () => {
    ...
    const first = nestedSignal(parent, 'first', {debugName: 'first'});
    const last = nestedSignal(parent, 'last', {debugName: 'last'});
  ...
  it('deeply blocks updates to unaffected parts of the tree', () => {
    ...
    const name = nestedSignal(parent, 'name', {debugName: 'name'});
    const first = nestedSignal(name, 'first', {debugName: 'first'});
    const last = nestedSignal(name, 'last', {debugName: 'last'});
    const status = nestedSignal(parent, 'status', {debugName: 'status'});
  ...
  it('updates siblings targeting the same property', () => {
    const parent = signal({first: 'John', last: 'Wick'});
    const first1 = nestedSignal(parent, 'first', {debugName: 'first1'});
    const first2 = nestedSignal(parent, 'first', {debugName: 'first2'});
  ...
  it('blocks updates when the property name is dynamic but stable', () => {
    ...
    const dynamic = nestedSignal(parent, property, {debugName: 'dynamic'});

    const age = nestedSignal(parent, 'age', {debugName: 'age'});
  ...
  describe('structuralSignal', () => {
    it('tracks only non-nested signal changes', () => {
