...
import {Content, Element, Reference, Template, Variable, Visitor, TextAttribute, BoundAttribute, BoundEvent, BoundText, Text, Node} from '../r3_ast';
...
export class R3TargetBinder<D> implements TargetBinder<D> {
  ...
  bind(target: Target): BoundTarget<D> {
    if (target.template) {
      ...
      const {directives, references} = DirectiveMatcher.apply(target.template, this.directiveMatcher);
      const {expressions, symbols, nestingLevel} = TemplateBinder.apply(target.template, scope);
      return new R3BoundTarget<D>(target, directives, references, expressions, symbols, nestingLevel);
    }
...
class DirectiveMatcher<D> implements Visitor {
  constructor(private matcher: SelectorMatcher<Directive<D>>, private directives: Map<Element|Template, Directive<D>[]>, private references: Map<BoundAttribute|BoundEvent|Reference, Directive<D>|Element|Template>) {}
  ...
  static apply<D>(template: Node[], selectorMatcher: SelectorMatcher<Directive<D>>): {
    ...
    const references = new Map<BoundAttribute|BoundEvent|Reference, Directive<D>|Element|Template>();
...
export class R3BoundTarget<D> implements BoundTarget<D> {
  constructor(
    ...
    private references: Map<BoundAttribute|BoundEvent|Reference|TextAttribute, Directive<D>|Element|Template>,
  ...
  getConsumerOfBinding(binding: BoundAttribute|BoundEvent|TextAttribute): Directive<D>|Element|Template|null {
