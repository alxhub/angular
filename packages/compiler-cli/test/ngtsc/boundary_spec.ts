...
runInEachFileSystem(() => {
  describe('ngtsc @boundary type checking', () => {
    ...
    it('should type check error alias as Error', () => {
      ...
      if (diags.length > 0) {
        console.log("DIAGS for should type check error alias as Error:", JSON.stringify(diags.map(d => typeof d.messageText === 'string' ? d.messageText : d.messageText.messageText)));
      }
    ...
    it('should error when accessing non-existent properties on error alias', () => {
      ...
      expect(diags[0].messageText).toContain("Property 'nonExistentProperty' does not exist on type 'Error'");
    });
    ...
    it('should narrowing type using condition when available', () => {
      ...
      const diags = env.driveDiagnostics();
      if (diags.length > 0) {
        console.log("DIAGS for should narrowing type using condition when available:", JSON.stringify(diags.map(d => typeof d.messageText === 'string' ? d.messageText : d.messageText.messageText)));
      }
      expect(diags.length).toBe(0);
    });
  });
