...
class AngularCompilerProgram implements Program {
  ...
  getTsSemanticDiagnostics(sourceFile?: ts.SourceFile, cancellationToken?: ts.CancellationToken):
    ...
    if (this.ngtscAdapter !== null && diags.length === 0) {
      diags.push(...this.ngtscAdapter.getTsDiagnostics(this.tsProgram));
    }
    return diags;
  }
  ...
  private _createProgramWithBasicStubs(): {
    ...
    if (!!this.options.ivyBridgeEnabled) {
      this.enableIvyBridge(tmpProgram);
    }
  ...
  private _updateProgramWithTypeCheckStubs(
    ...
    debugger;
    if (this.ngtscAdapter !== null && this.ngtscAdapter.typeCheckFilePath !== null) {
      rootFiles.push(this.ngtscAdapter.typeCheckFilePath);
      this.hostAdapter.enableIvyBridgeTemplateTypeChecking(this.ngtscAdapter);
    }
    this._tsProgram = ts.createProgram(rootFiles, this.options, this.hostAdapter, tmpProgram);
  ...
  private enableIvyBridge(program: ts.Program): void {
    ...
    this.ngtscAdapter = new IvyBridge(
        this.hostAdapter, program, symResolver, metaResolver,
