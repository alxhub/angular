...
export class TsCompilerAotCompilerTypeCheckHostAdapter implements ts.CompilerHost, AotCompilerHost,
  ...
  getSourceFile(
    ...
    if (this.ivyBridge !== null && fileName === this.ivyBridge.typeCheckFilePath) {
      debugger;
      const file = this.ivyBridge.transformFileForTemplateTypeChecking(
          ts.createSourceFile(this.ivyBridge.typeCheckFilePath, '', ts.ScriptTarget.Latest));
      return file;
    }
