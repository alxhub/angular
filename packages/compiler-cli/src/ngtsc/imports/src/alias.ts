 ...
 *
 * See the README.md for more information how aliasing works within the compiler.
 */
export interface AliasingHost {
   ...
   */
  maybeAliasSymbolAs(
      decl: ClassDeclaration, context: ts.SourceFile, ngModuleName: string,
...
export class FileToModuleAliasingHost implements AliasingHost {
  ...
  maybeAliasSymbolAs(
    ...
    return this.aliasName(decl, context);
  }
...
export class PrivateExportAliasingHost implements AliasingHost {
   ...
   * emitting these exporst in .d.ts is a requirement for the `PrivateExportAliasingHost` to
   * function correctly.
   */
  readonly aliasExportsInDts = true;

  maybeAliasSymbolAs(decl: ClassDeclaration, context: ts.SourceFile, ngModuleName: string): string
      |null {
    // Look for a user-provided export of `decl` in `context`. If one exists.
    ...
    const exportedClasses = new Set<ClassDeclaration>();
    exports.forEach(value => {
      if (value.node !== null && isNamedClassDeclaration(value.node)) {
        exportedClasses.add(value.node);
      }
    });
    // TODO(alxhub): maybe add a host method to check for the existence of an export without going
    // through the allocation of a `Set` here.
    if (exportedClasses.has(decl)) {
      // The module exports the declared class directly, no alias is necessary.
      return null;
    }
    return `ɵngExportɵ${ngModuleName}ɵ${decl.name.text}`;
  }
