...
export class SymbolBuilder {
  ...
  private getSymbolOfBoundEvent(eventBinding: TmplAstBoundEvent): OutputBindingSymbol|null {
    ...
    if (consumer instanceof TmplAstTemplate || consumer instanceof TmplAstElement) {
      ...
      const tsSymbol = typeChecker.getSymbolAtLocation(addEventListener);
      const tsType = typeChecker.getTypeAtLocation(addEventListener);
      ...
      const tsSymbol = typeChecker.getSymbolAtLocation(outputFieldAccess.argumentExpression);
