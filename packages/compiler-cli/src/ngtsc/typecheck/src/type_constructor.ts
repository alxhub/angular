...
export function generateTypeCtorDeclarationFn(
    ...
    nodeTypeRef: ts.Identifier | ts.QualifiedName): ts.FunctionDeclaration {
  ...
  console.error('type ctor for', node.name.text, 'has body', meta.body);
  const body = meta.body ?
      ts.createBlock([ts.createReturn(ts.createNonNullExpression(ts.createNull()))]) :
      undefined;
