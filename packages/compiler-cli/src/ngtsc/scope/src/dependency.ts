...
export class MetadataDtsModuleScopeResolver implements DtsModuleScopeResolver {
  ...
  constructor(private metaReader: MetadataReader, private aliasGenerator: AliasGenerator|null) {}
  ...
  resolve(ref: Reference<ClassDeclaration>): ExportScope|null {
    ...
    const meta = this.metaReader.getNgModuleMetadata(ref);
    if (meta === null) {
    ...
    for (const exportRef of meta.exports) {
      ...
      const directive = this.metaReader.getDirectiveMetadata(exportRef);
      if (directive !== null) {
      ...
      const pipe = this.metaReader.getPipeMetadata(exportRef);
      if (pipe !== null) {
        if (!declarations.has(exportRef.node)) {
