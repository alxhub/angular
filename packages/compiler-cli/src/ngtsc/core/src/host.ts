...
import {FactoryGenerator, FactoryTracker, isShim, ShimHostAdapter, SummaryGenerator} from '../../shims';
...
export class NgCompilerHost extends DelegatingCompilerHost implements
  ...
  constructor(
      ...
      rootDirs: ReadonlyArray<AbsoluteFsPath>, private shimAdapter: ShimHostAdapter,
      entryPoint: AbsoluteFsPath|null, factoryTracker: FactoryTracker|null,
  ...
  static wrap(
    ...
    const shimHostAdapter =
        ShimHostAdapter.create(topLevelShimGenerators, perFileShimGenerators, oldProgram);
    return new NgCompilerHost(
        delegate, inputFiles, rootDirs, shimHostAdapter, entryPoint, factoryTracker, diagnostics);
  }
  ...
  getSourceFile(
    ...
    const shimSf = this.shimAdapter.maybeGetShim(
        absoluteFsPath,
        (reqFileName: string) => this.delegate.getSourceFile(
            reqFileName, languageVersion, onError, shouldCreateNewSourceFile));
    if (shimSf !== null) {
      // Yes, so just return it.
      return shimSf;
    }

    // No, so it's a file which might need shims.
    const sf =
    ...
    this.shimAdapter.maybeAddShims(sf, absoluteFsPath);
    return sf;
  }
  ...
  fileExists(fileName: string): boolean {
    ...
    return this.delegate.fileExists(fileName) || this.shimAdapter.isAddedShim(resolve(fileName));
  }
