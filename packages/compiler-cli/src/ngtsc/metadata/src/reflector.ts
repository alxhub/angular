...
export interface Decorator {
  ...
  /**
   * Any arguments of a call expression, if one is present.
   */
  args: ts.Expression[];
}
 ...
 */
export function reflectConstructorParameters(node: ts.ClassDeclaration, checker: ts.TypeChecker): Parameter[]|null {
...
function reflectParameter(node: ts.ParameterDeclaration, checker: ts.TypeChecker): Parameter {
  ...
  const decorators = node.decorators && node
    .decorators
    .map(decorator => reflectDecorator(decorator, checker))
...
export function reflectDecorator(decorator: ts.Decorator, checker: ts.TypeChecker): Decorator|null {
  ...
  // Look up the declaration of the decorator. It should map to a single declaration, otherwise the
  // decorator can't be resolved statically.
  const symbol = checker.getSymbolAtLocation(decoratorOfInterest);
