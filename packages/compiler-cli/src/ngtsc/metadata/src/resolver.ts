...
export class ResolvedReference<T extends ts.Node = ts.Node> extends Reference<T> {
  ...
  toExpression(context: ts.SourceFile, importMode: ImportMode = ImportMode.USE_EXISTING_IMPORT): Expression {
...
export function staticallyResolve(
    ...
    foreignFunctionResolver?: (node: Reference<ts.FunctionDeclaration | ts.MethodDeclaration>, args: ts.Expression[]) =>
...
interface Context {
  ...
  scope: Scope;
  foreignFunctionResolver?(ref: Reference<ts.FunctionDeclaration | ts.MethodDeclaration>, args: ReadonlyArray<ts.Expression>): ts.Expression|null;
}
...
class StaticInterpreter {
  ...
  private visitCallExpression(node: ts.CallExpression, context: Context): ResolvedValue {
    ...
    if (fn.body === undefined) {
      ...
      if (expr === null) {
        throw new Error(`could not resolve foreign function declaration: ${node.getSourceFile().fileName} ${(lhs.node.name as ts.Identifier).text}`);
      }
...
function isFunctionOrMethodReference(ref: Reference<ts.Node>): ref is Reference<ts.FunctionDeclaration|
