...
import {reflectConstructorParameters, Parameter} from '../src/reflector';

describe('reflector', () => {
  describe('ctor params', () => {
    it('reflects a single argument', () => {
    ...
    it('reflects a decorated argument', () => {
      const program = makeProgram([
        {name: 'dec.ts', contents: `
    ...
    it('reflects a decorated argument with a call', () => {
      const program = makeProgram([
        {name: 'dec.ts', contents: `
          export function dec(target: any, key: string, index: number) {
          }
    ...
    it('reflects a decorated argument with an indirection', () => {
      const program = makeProgram([
        {name: 'bar.ts', contents: `
          export class Bar {}
        `},
        {name: 'entry.ts', contents: `
            import {Bar} from './bar';
...
function expectArgument(arg: Parameter, name: string, type?: string, decorator?: string, decoratorFrom?: string): void {
  ...
  if (decorator !== undefined) {
    ...
    expect(arg.decorators.some(dec => dec.name === decorator && dec.from === decoratorFrom)).toBe(true);
  }
}
