...
import {staticallyResolve, StaticValue} from '../src/resolver';

function makeSimpleProgram(contents: string): ts.Program {
  return makeProgram([{name: 'entry.ts', contents}]);
}

function makeExpression(code: string, expr: string): {expression: ts.Expression, checker: ts.TypeChecker} {
...
function evaluate(code: string, expr: string): StaticValue {
  const {expression, checker} = makeExpression(code, expr);
  return staticallyResolve(expression, checker);
}
...
describe('ngtsc metadata', () => {
  ...
  it('static property call works', () => {
    expect(evaluate(`class Foo { static bar(test) { return test; } }`, 'Foo.bar("test")')).toEqual('test')
  });
  it('indirected static property call works', () => {
    expect(evaluate(`class Foo { static bar(test) { return test; } }; const fn = Foo.bar;`, 'fn("test")')).toEqual('test')
  });
  it('array works', () => {
    expect(evaluate(`const x = 'test'; const y = [1, x, 2];`, 'y')).toEqual([1, 'test', 2]);
  });
  it('array spread works', () => {
    expect(evaluate(`const a = [1, 2]; const b = [4, 5]; const c = [...a, 3, ...b];`, 'c')).toEqual([1, 2, 3, 4, 5]);
  });
  it('&& operations work', () => {
  ...
  it('chain of re-exports works', () => {
    const program = makeProgram([
      ...
      {name: 'entry.ts', contents: `import * as mod from './direct-reexport'; const target$ = mod.value.property;`},
    ]);
    const checker = program.getTypeChecker();
    const result = getDeclaration(program, 'entry.ts', 'target$', ts.isVariableDeclaration);
    const expr = result.initializer!;
  ...
  it('map spread works', () => {
    const map: Map<string, number> = evaluate(`const a = {a: 1}; const b = {b: 2, c: 1}; const c = {...a, ...b, c: 3};`, 'c') as Map<string, number>;
